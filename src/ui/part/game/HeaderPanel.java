...
public class HeaderPanel extends JPanel implements MouseListener, LeapListener {
	...
	public HeaderPanel() {
		...
		LeapManager.getDefaultLeapManager().addLeapListener(startGame, this);
	...
	public void setNotice(NoticeType type, String notice) {
		if (type == NoticeType.SHOW_TOPIC) {
			...
			AudioManager.getDefaultAudioManager().hint();
		}
	...
	public void setTime(int time) {
		if (time == 0) {
			AudioManager.getDefaultAudioManager().showTopic();
		} else if (time < 10) {
			AudioManager.getDefaultAudioManager().countdown();
		}
