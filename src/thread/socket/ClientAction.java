...
import ui.face.GamePanel;
import ui.face.MainFrame;
import ui.part.game.HeaderPanel.NoticeType;
	...
	public void run() {
		...
		while (!client.isClosed()) {
			try {
				...
				switch (dtpg.getType()) {
				case LOGIN:
					AudioManager.getDefaultAudioManager().login();
				...
				case PAINTER:
					...
					if (Config.progressId.equals(painter.getProgressId())) {
						gamePanel.getHeaderPanel().setNotice(NoticeType.SHOW_PAINTER, painter.getNickName());
						gamePanel.getHeaderPanel().setPainterHeadPortrait(painter.getHeadPortrait());
					}
					gamePanel.getScorePanel().setPainter(painter);
					break;
				case TOPIC:
					gamePanel.getHeaderPanel().setNotice(NoticeType.SHOW_TOPIC, dtpg.getData().toString());
					break;
				case DRAWABLE:
				...
				case GAME_TIME:
					...
					break;
				case HINT:
					gamePanel.getHeaderPanel().setNotice(NoticeType.SHOW_HINT, dtpg.getData().toString());
					break;
