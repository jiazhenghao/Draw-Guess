...
public class GamePlayThread extends Thread {
						...
						if (time > showTopicTime + drawTime) {				//µ¹¼ÆÊ±½×¶Î
							...
							if (time == showTopicTime + drawTime - showHintTime) {
								if (client == null) {
									...
									gamePanel.getHeaderPanel().setNotice(NoticeType.SHOW_HINT, topic[0].length() + "¸ö×Ö");
								...
								if (client == null) {
									...
									gamePanel.getHeaderPanel().setNotice(NoticeType.SHOW_HINT, topic[0].length() + "¸ö×Ö£¬" + topic[1]);
	...
	private ArrayList<String> getTopics() {
		...
		try {
			InputStream in = FileUtil.getInputStream("file", "topics");
